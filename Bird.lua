...
function Bird:collides(pipe)
    ...
    if (self.x + SPACE) + (self.width - 2 * SPACE) >= pipe.x and self.x + SPACE <= pipe.x + PIPE_WIDTH then
        if (self.y + SPACE) + (self.height - 2*SPACE) >= pipe.y and self.y + SPACE <= pipe.y + PIPE_HEIGHT then
            return true
        end
    end
